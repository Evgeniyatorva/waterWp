<?php get_header(); ?>

		<main class="main">
			<section class="section header__img">
				<div class="container">
					<div class="row">
						<div class="header__title">
							<h1>Бурение скважин на воду<br> <span>«Под ключ»</span></h1>
							<p>- Высокое качество</p>
							<p>- Низкие цены</p>
							<p>- Довольные клиенты</p>
						</div>
						<form method="post" class="form__action form__margin_header" id="form1">
							<h3>Оставьте Вашу онлайн заявку:</h2>
							<input class="form__name" type="text" placeholder="Ваше имя" name="name">
							<input class="form__tel" type="tel" placeholder="Ваш номер телефона" minlength="7" maxlength="13" name="phone">
							<input type="submit" value="ЗАКАЗАТЬ">
						</form>
					</div>
					<div class="row">
						<div class="header__tagline">
							<h3>Внимательность к деталям, мастерство и надежная команда - путь к успеху в любом деле!</h3>
						</div>
					</div>
				</div>
			</section>
			<section class="advantage section-color_white">
				<div class="container">
					<div class="row">
						<div class="section__title section__title_black">
							<h2 id="advantage">Почему выбирают именно нас</h2>
						</div>
					</div>
					<div class="row">
						<div class="advantage__item">
							<div class="item">
								<div class="item__img">
									<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
										 viewBox="0 0 486.742 486.742" style="enable-background:new 0 0 486.742 486.742;" xml:space="preserve">
									<g>
										<g>
											<path d="M33,362.371v78.9c0,4.8,3.9,8.8,8.8,8.8h61c4.8,0,8.8-3.9,8.8-8.8v-138.8l-44.3,44.3
												C57.9,356.071,45.9,361.471,33,362.371z"/>
											<path d="M142,301.471v139.8c0,4.8,3.9,8.8,8.8,8.8h61c4.8,0,8.8-3.9,8.8-8.8v-82.3c-13.9-0.3-26.9-5.8-36.7-15.6L142,301.471z"/>
											<path d="M251,350.271v91c0,4.8,3.9,8.8,8.8,8.8h61c4.8,0,8.8-3.9,8.8-8.8v-167.9l-69.9,69.9
												C257,345.971,254.1,348.271,251,350.271z"/>
											<path d="M432.7,170.171l-72.7,72.7v198.4c0,4.8,3.9,8.8,8.8,8.8h61c4.8,0,8.8-3.9,8.8-8.8v-265.6c-2-1.7-3.5-3.2-4.6-4.2
												L432.7,170.171z"/>
											<path d="M482.6,41.371c-2.9-3.1-7.3-4.7-12.9-4.7c-0.5,0-1.1,0-1.6,0c-28.4,1.3-56.7,2.7-85.1,4c-3.8,0.2-9,0.4-13.1,4.5
												c-1.3,1.3-2.3,2.8-3.1,4.6c-4.2,9.1,1.7,15,4.5,17.8l7.1,7.2c4.9,5,9.9,10,14.9,14.9l-171.6,171.7l-77.1-77.1
												c-4.6-4.6-10.8-7.2-17.4-7.2c-6.6,0-12.7,2.6-17.3,7.2L7.2,286.871c-9.6,9.6-9.6,25.1,0,34.7l4.6,4.6c4.6,4.6,10.8,7.2,17.4,7.2
												s12.7-2.6,17.3-7.2l80.7-80.7l77.1,77.1c4.6,4.6,10.8,7.2,17.4,7.2c6.6,0,12.7-2.6,17.4-7.2l193.6-193.6l21.9,21.8
												c2.6,2.6,6.2,6.2,11.7,6.2c2.3,0,4.6-0.6,7-1.9c1.6-0.9,3-1.9,4.2-3.1c4.3-4.3,5.1-9.8,5.3-14.1c0.8-18.4,1.7-36.8,2.6-55.3
												l1.3-27.7C487,49.071,485.7,44.571,482.6,41.371z"/>
										</g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									</svg>
								</div>
								<div class="item__text">
									<p><span>12 </span>лет на рынке</p>
								</div>
							</div>
							<div class="item">
								<div class="item__img">
									<svg version="1.1" x="0px" y="0px" viewBox="0 0 512 512">
									<g>
										<g>
											<rect x="182" y="466" width="150" height="45"/>
										</g>
									</g>
									<g>
										<g>
											<rect x="272" y="391" width="210" height="45"/>
										</g>
									</g>
									<g>
										<g>
											<rect x="30" y="391" width="212" height="45"/>
										</g>
									</g>
									<g>
										<g>
											<path d="M30,466v30c0,8.291,6.709,15,15,15h107v-45H30z"/>
										</g>
									</g>
									<g>
										<g>
											<path d="M362,466v45h105c8.291,0,15-6.709,15-15v-30H362z"/>
										</g>
									</g>
									<g>
										<g>
											<path d="M497,301h-75V61h15c8.276,0,15,6.724,15,15v17.948c0,21.159,10.645,41.582,29.045,52.029
												C515.768,165.693,521.233,121,497,121c-8.276,0-15-6.724-15-15V76c0-24.853-20.147-45-45-45h-15V16c0-19.773-30-19.854-30,0v15
												h-32.763C353.024,13.578,336.53,1,317,1H197c-19.53,0-36.024,12.578-42.237,30H120V16c0-19.773-30-19.854-30,0v285H15
												c-8.284,0-15,6.716-15,15v30c0,8.284,6.716,15,15,15h482c8.284,0,15-6.716,15-15v-30C512,307.716,505.284,301,497,301z M392,301
												H120V61h34.763C160.976,78.422,177.47,91,197,91h45v60h-45c-9.308,0-16.32,8.403-14.795,17.461l15,90
												C198.406,265.697,204.661,271,212,271h90c7.339,0,13.594-5.303,14.795-12.539l15-90C333.32,159.403,326.308,151,317,151h-45V91h45
												c19.53,0,36.024-12.578,42.237-30H392V301z"/>
										</g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									</svg>
								</div>
								<div class="item__text">
									<p>Более<span> 1500<br></span>пробуренных скважин</p>
								</div>
							</div>
							<div class="item">
								<div class="item__img">
									<svg version="1.1" x="0px" y="0px" viewBox="0 0 512 512">
									<g>
										<g>
											<path d="M175.394,65.229c1.697,20.734,19.049,37.034,40.221,37.034c21.171,0,38.524-16.301,40.221-37.034H175.394z"/>
										</g>
									</g>
									<g>
										<g>
											<path d="M261.237,42.607h-2.302c-3.912-14.769-15.177-26.536-29.637-31.169v10.05c0,2.595-2.104,4.699-4.699,4.699h-3.637V5.558
												c0-3.069-2.489-5.558-5.557-5.558c-3.069,0-5.558,2.489-5.558,5.558v20.631h-3.637c-2.595,0-4.699-2.104-4.699-4.699v-9.914
												c-14.257,4.726-25.339,16.408-29.214,31.032h-2.725c-3.178,0-5.753,2.575-5.753,5.753c-0.001,3.178,2.574,5.753,5.752,5.753
												c42.056,0,50.049,0,91.667,0c3.178,0,5.753-2.575,5.753-5.753S264.414,42.607,261.237,42.607z"/>
										</g>
									</g>
									<g>
										<g>
											<path d="M394.857,89.761c-0.8-1.477-2.183-2.553-3.811-2.965l-17.389-4.407c-1.63-0.412-3.357-0.126-4.764,0.793
												c-1.407,0.918-2.367,2.382-2.648,4.039l-13.051,77.325c-3.398,0.457-6.719,1.795-9.587,4.098l-29.537,23.734l0.029-31.267
												c0-0.039,0-0.077,0-0.116c-0.127-25.289-20.804-45.864-46.094-45.864h-21.758c-2.534,6.621-20.488,53.526-22.955,59.97
												l5.429-25.569c0.381-1.795,0.104-3.668-0.78-5.276l-7.454-13.557l6.627-12.052c0.987-1.795-0.313-3.998-2.364-3.998h-18.188
												c-2.049,0-3.352,2.201-2.364,3.998l6.627,12.052l-7.482,13.607c-0.868,1.578-1.151,3.411-0.8,5.177l4.746,25.618
												c-4.342-11.134-18.321-48.7-22.715-59.969h-21.344c-25.29,0-45.967,20.574-46.094,45.864l-0.719,143.1
												c-0.054,10.757,8.623,19.521,19.38,19.576c0.033,0,0.067,0,0.1,0c10.712,0,19.422-8.656,19.476-19.381l0.719-143.1
												c0.011-2.132,1.744-3.851,3.875-3.846c2.131,0.006,3.854,1.734,3.854,3.865l0.009,327.417c0,12.909,10.465,23.374,23.374,23.374
												s23.374-10.465,23.374-23.374v-186.83h10.092v186.83c0,12.909,10.465,23.374,23.374,23.374c12.909,0,23.374-10.465,23.374-23.374
												c0-309.072-0.413-132.171-0.423-327.342c0-2.225,1.784-4.039,4.008-4.077c2.224-0.038,4.069,1.715,4.144,3.938
												c0,0.001,0,0.001,0,0.002l-0.067,71.879c-0.007,7.498,4.292,14.334,11.052,17.578c6.788,3.257,14.805,2.301,20.627-2.378
												l30.113-24.196l-20.514,60.047c-1.136,3.323,0.809,6.915,4.216,7.778l17.391,4.407c3.404,0.863,6.827-1.366,7.411-4.832
												l14.495-85.881l8.142-6.542c8.385-6.738,9.721-18.999,2.983-27.384c-0.47-0.585-0.973-1.127-1.493-1.643l25.759-75.408
												C395.804,92.983,395.658,91.238,394.857,89.761z"/>
										</g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									</svg>
								</div>
								<div class="item__text">
									<p>Высококлассные<br> специалисты</p>
								</div>
							</div>
							<div class="item">
								<div class="item__img">
									<svg version="1.1" x="0px" y="0px" viewBox="0 0 512 512">
									<g>
										<g>
											<path d="M226.932,399.948c-19.96,18.445-47.567,22.576-72.053,10.786c-8.852-4.263-16.322-10.149-22.17-17.199l-33.341,73.744
												c-1.517,3.355,0.177,5.884,0.975,6.815c0.798,0.93,3.039,2.989,6.585,2.003l24.272-6.756c2.766-0.769,5.562-1.14,8.319-1.14
												c11.631,0,22.578,6.583,27.849,17.492l10.962,22.685c1.601,3.315,4.604,3.646,5.854,3.621c1.226-0.016,4.242-0.414,5.758-3.769
												l53.033-117.304C237.148,392.603,231.63,395.606,226.932,399.948z"/>
										</g>
									</g>
									<g>
										<g>
											<path d="M412.631,467.279l-33.341-73.744c-5.848,7.051-13.318,12.937-22.17,17.199c-24.487,11.79-52.093,7.659-72.053-10.786
												c-4.698-4.342-10.216-7.345-16.045-9.022l53.033,117.304c1.517,3.356,4.533,3.753,5.758,3.769c1.25,0.025,4.253-0.306,5.854-3.621
												l10.962-22.685c5.27-10.909,16.218-17.492,27.849-17.492c2.757,0,5.554,0.371,8.319,1.14l24.272,6.756
												c3.546,0.987,5.788-1.072,6.585-2.003C412.454,473.162,414.148,470.633,412.631,467.279z"/>
										</g>
									</g>
									<g>
										<g>
											<path d="M438.821,207.791c-27.69-18.96-36.282-56.605-19.56-85.702c10.051-17.491,4.82-34.775-3.427-45.118
												c-8.248-10.34-23.936-19.285-43.223-13.38c-32.084,9.827-66.877-6.925-79.201-38.141C286.002,6.686,269.227,0,256,0
												c-13.227,0-30.002,6.686-37.41,25.451c-12.324,31.217-47.114,47.967-79.201,38.141c-19.289-5.904-34.974,3.039-43.223,13.38
												c-8.247,10.343-13.478,27.625-3.427,45.118c16.722,29.096,8.13,66.742-19.56,85.702c-16.646,11.399-19.431,29.24-16.489,42.136
												c2.942,12.896,13.194,27.761,33.137,30.808c33.174,5.068,57.248,35.256,54.809,68.727c-1.468,20.121,10.745,33.423,22.662,39.163
												c11.918,5.739,29.932,6.995,44.748-6.698c12.322-11.387,28.141-17.083,43.953-17.083c15.818,0,31.628,5.693,43.952,17.083
												c14.818,13.694,32.833,12.438,44.75,6.698c11.917-5.739,24.129-19.041,22.662-39.162c-2.439-33.471,21.635-63.659,54.809-68.728
												c19.943-3.047,30.193-17.913,33.137-30.808C458.252,237.03,455.465,219.189,438.821,207.791z M256,335.923
												c-72.575,0-131.619-59.044-131.619-131.619S183.424,72.684,256,72.684c72.576,0,131.619,59.044,131.619,131.619
												C387.618,276.878,328.575,335.923,256,335.923z"/>
										</g>
									</g>
									<g>
										<g>
											<path d="M255.999,97.225c-59.044,0-107.079,48.036-107.079,107.079c0,59.043,48.034,107.079,107.079,107.079
												s107.079-48.036,107.079-107.079S315.043,97.225,255.999,97.225z M310.874,193.922l-66.642,48.675
												c-2.115,1.545-4.653,2.362-7.237,2.362c-0.666,0-1.335-0.054-2.001-0.164c-3.249-0.537-6.147-2.358-8.041-5.054l-19.934-28.382
												c-3.895-5.547-2.556-13.2,2.989-17.095c5.546-3.895,13.198-2.557,17.094,2.989l12.75,18.154l56.548-41.302
												c5.473-3.995,13.15-2.803,17.146,2.671C317.543,182.248,316.346,189.924,310.874,193.922z"/>
										</g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									<g>
									</g>
									</svg>

								</div>
								<div class="item__text">
									<p><span>100% </span>гарантия<br> качества</p>
								</div>
							</div>
						</div>
					</div>
				</div>
			</section>
			<section class="why section-color_blue">
				<div class="container">
					<div class="row">
						<div class="section__title section__title_white">
							<h2>Для чего нужна скважина</h2>
						</div>
					</div>
					<div class="row">
						<div class="why__item">
							<div class="item">
								<img src="<?php echo get_template_directory_uri() ?>/img/why/1.jpg" alt="Колодец">
								<p>Постоянный доступ к воде - не роскошь, а основа нормальной жизни! Имея скважину на своем участке Вы решаете эту проблему. И вам больше не нужно ходить за несколько километров до живительного источника.</p>
							</div>
							<div class="item">
								<img src="<?php echo get_template_directory_uri() ?>/img/why/2.jpg" alt="Вода">
								<p>Благодаря очистительным фильтрам вы всегда будете получать только свежую и кристально чистую воду.</p>
							</div>
							<div class="item">
								<img src="<?php echo get_template_directory_uri() ?>/img/why/3.jpg" alt="Экономия">
								<p>Несмотря на первичные растраты на бурение и обустройство скважины, в дальнейшем, вы окупите все Ваши вложения.</p>
							</div>
						</div>
					</div>
				</div>
			</section>
			<section class="order section-color_white">
				<div class="container">
					<div class="row">
						<div class="section__title section__title_black">
							<h2>Оставьте Вашу заявку и узнайте точную сумму установки скважины</h2>
						</div>
					</div>
					<div class="row">
						<form method="post" class="order__form" id="form2">
							<input class="form__name" type="text" placeholder="Ваше имя" name="name">
							<input class="form__tel" type="tel" placeholder="Ваш номер телефона" minlength="7" maxlength="13" name="phone">
							<input type="submit" value="ЗАКАЗАТЬ">
						</form>
					</div>
					<div class="row">
						<div class="order__tel">
							<h3>или свяжитесь с нами по телефону</h3>
							<a href="#">+375447654336</a>
						</div>
					</div>
				</div>
			</section>
			<section class="work section-color_blue">
				<div class="container">
					<div class="row">
						<div class="section__title section__title_white">
							<h2>Мы работаем по всей территории Республики Беларусь</h2>
						</div>
					</div>
					<div class="row">
						<div class="work__img">
							<img src="<?php echo get_template_directory_uri() ?>/img/pb.png" alt="pb">
							<img src="<?php echo get_template_directory_uri() ?>/img/car.png" alt="car">
						</div>
					</div>
				</div>
			</section>
			<section class="partner section-color_white">
				<div class="container">
					<div class="row">
						<div class="section__title section__title_black">
							<h2>Наши партнеры</h2>
						</div>
					</div>
					<div class="row">
						<div class="slider" id="slider">
							<div class="slider__gallery">
								<ul>
									<li><img src="<?php echo get_template_directory_uri() ?>/img/partners/1.png" alt="al-ko"></li>
									<li><img src="<?php echo get_template_directory_uri() ?>/img/partners/2.jpg" alt="fraste"></li>
									<li><img src="<?php echo get_template_directory_uri() ?>/img/partners/3.png" alt="ibo"></li>
									<li><img src="<?php echo get_template_directory_uri() ?>/img/partners/4.jpg" alt="pedrollo"></li>
									<li><img src="<?php echo get_template_directory_uri() ?>/img/partners/5.jpg" alt="pedrollo"></li>
									<li><img src="<?php echo get_template_directory_uri() ?>/img/partners/6.png" alt="pedrollo"></li>
									<li><img src="<?php echo get_template_directory_uri() ?>/img/partners/7.jpg" alt="patriot"></li>
									<li><img src="<?php echo get_template_directory_uri() ?>/img/partners/8.png" alt="unipump"></li>
								</ul>
							</div>
							<button class="slider__prev"></button>
							<button class="slider__next"></button>
						</div>
					</div>
				</div>
			</section>
			<section class="articles section-color_blue">
				<div class="container">
					<div class="row">
						<div class="section__title section__title_white">
							<h2>Статьи</h2>
						</div>
					</div>
					<div class="row">
						<div class="articles__item">
						<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
						<div class="article">
								<a href="<?php the_permalink(); ?>"><?php the_post_thumbnail(); ?></a>
								<h3><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
								<p><?php the_excerpt(); ?></p>
								<time><?php the_time('j F Y'); ?></time>
								<a class="button" href="<?php the_permalink(); ?>">Читать далее</a>
							</div>	

						<?php endwhile; ?>
						<?php endif; ?>


						
							<!-- <div class="article">
								<img src="<?php echo get_template_directory_uri() ?>/img/art.jpg" alt="#">
								<h3>Заголовок статьи №1</h3>
								<p>Это текст статьи и это текст статьи и это текст статьи и это текст статьи и это текст статьи  и это текст статьи и это текст статьи и это текст</p>
								<time>12-03-2018</time>
								<a href="article_page.html">Читать далее</a>
							</div>	
							<div class="article">
								<img src="<?php echo get_template_directory_uri() ?>/img/art.jpg" alt="#">
								<h3>Заголовок статьи №1</h3>
								<p>Это текст статьи и это текст статьи и это текст статьи и это текст статьи и это текст статьи  и это текст статьи и это текст статьи и это текст</p>
								<time>12-03-2018</time>
								<a href="article_page.html">Читать далее</a>
							</div>	
							<div class="article">
								<img src="<?php echo get_template_directory_uri() ?>/img/art.jpg" alt="#">
								<h3>Заголовок статьи №1</h3>
								<p>Это текст статьи и это текст статьи и это текст статьи и это текст статьи и это текст статьи  и это текст статьи и это текст статьи и это текст</p>
								<time>12-03-2018</time>
								<a href="article_page.html">Читать далее</a>
							</div> -->
						</div>														
					</div>
				</div>
			</section>
			<section class="action section-color_white">
				<div class="container">
					<div class="row">
						<div class="section__title">
							<h2 id="action">Заказать</h2>
						</div>
					</div>
					<div class="row">
						<form method="post" class="form__action form__margin_footer" id="form3">
							<h3>Закажите Вашу скважину прямо сейчас!</h3>
							<input class="form__name" type="text" placeholder="Ваше имя" name="name">
							<input class="form__tel" type="tel" placeholder="Ваш номер телефона" minlength="7" maxlength="13" name="phone">
							<input type="submit" value="ЗАКАЗАТЬ">
						</form>
						<img class="action__img" src="<?php echo get_template_directory_uri() ?>/img/build.png" alt="build">
					</div>
				</div>
			</section>
		</main>

<?php get_footer(); ?>

